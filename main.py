...
from sharepoint_upload import upload_file
import os
...
# CLIENT_CERT = "/app/certs/certificate.pem"
# CLIENT_KEY = "/app/certs/pri-key.pem"
...
# === Functions ===
def get_monitoring_list_ids(session):
    url = urljoin(API_BASE_URL, "common/monitoring_lists")
    response = session.get(url)
    response.raise_for_status()
    return response.json()

def get_monitoring_list_details(list_id, session):
...
def get_monitoring_list_components(list_id, session):
...
def get_component_details(comp_id, session):
...
def unique_component_usages_with_details(session):
    component_usage = defaultdict(int)
    details_dict = {}
    comp_id_to_list_ids = defaultdict(list)
    for list_id in get_monitoring_list_ids(session):
        list_data = get_monitoring_list_details(list_id, session)
        name = list_data.get("name", "Unknown List")
        creation_date = list_data.get("creation_date", "Unknown Date")

        component_ids = get_monitoring_list_components(list_id, session)
        unique_components_in_list = set(component_ids)
        for comp_id in unique_components_in_list:
            ...
            component_usage[comp_id] += 1
            if comp_id not in details_dict:
                details = get_component_details(comp_id, session)
                if details:
                    details_dict[comp_id] = details
...
def export_to_excel(component_usage, filename="/app/monitoring_lists.xlsx"):
...
def export_to_json(data, filename="/app/monitoring_lists.json"):
...
def main(session):
    ...
    if unique_usage_dict:
        ...
        excel_path = f"monitoring_lists_{timestamp}.xlsx"
        json_path = f"monitoring_lists_{timestamp}.json"
